...
# Put this before load_data so you can pass the choice in
st.sidebar.header("Data Source Settings")
engine_choice = st.sidebar.selectbox(
    "Select Processing Engine:",
    ["SQL Pipeline", "Pandas Pipeline"]
)
...
def load_data(selection):
    ...
    # Load from DuckDB instead of CSV
...
# Pass the sidebar choice into the function
df = load_data(engine_choice)
...
# Convert keywords to uppercase once so it matches your clean_company column
agency_pattern = '|'.join([k.upper() for k in agency_keywords])

if filter_agencies:
    # This is now a very fast match because both sides are uppercase
    display_df = df[~df['clean_company'].str.contains(agency_pattern, na=False)]
else:
    display_df = df
...
with c2:
    st.subheader("Top Direct Client List")
...
num_rows = st.slider("Number of leads", 1, 10000, 100)

if not display_df.empty:
    # 1. Select the columns
    search_df = display_df[['clean_company', 'clean_title', 'business_tier', 'monthly_pay', 'est_commission', 'job_status']].head(num_rows).copy()
